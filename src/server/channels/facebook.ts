 ...
 * Exige Página do Facebook + App Review das permissões de mensagens.
 */
export const facebookAdapter: ChannelAdapter = {
  channel: "FACEBOOK",

  routingHint(payload: any): RoutingHint {
    return { pageId: payload?.entry?.[0]?.id };
  },
    ...
    for (const entry of payload?.entry ?? []) {
      for (const ev of entry?.messaging ?? []) {
        for (const mid of ev?.delivery?.mids ?? []) {
          out.push({ externalMessageId: mid, status: "DELIVERED" });
        }
      }
    }
    return out;
  ...
  async sendText(config: SendConfig, to: string, text: string) {
    ...
    if (!messageId) throw new Error("Messenger: resposta sem message_id.");
